...
target_data = torch.rand(8, 13, 1440, 720, 5)
target_surface_data = torch.rand(8, 1440, 720, 4)

optimizer = torch.optim.Adam(Pmodel.parameters(), lr=5e-4, weight_decay=3e-6)
...
for i in range(epochs):
    ...
    loader = DataLoader(list(zip(input_data,input_surface_data,target_data,target_surface_data)),shuffle=True, batch_size=1)
    for input_upper, input_surface, target_upper, target_surface in loader:
        ...
        print(input_upper.shape,input_surface.shape,target_upper.shape,target_surface.shape)
        output, output_surface = Pmodel(input_upper, input_surface)
        print(output.shape,output_surface.shape)#torch.Size([2, 5, 1440, 720, 14]) torch.Size([2, 4, 1440, 720])
        ...
        loss1 = criterion(output,target_upper)
        loss2 = criterion(output_surface,target_surface) * 0.25
        ...
        optimizer.step()
        optimizer.zero_grad()
        print('Epoch:{}, Loss_upper: {}, Loss_surface: {}'.format(i, loss1.item(), loss2.item()))
...
#SaveModel()
#torch.save(Pmodel, '/home/p131/pangu/save_test.pt')
torch.save(Pmodel.state_dict(), '/home/p131/pangu/save_test.pt')